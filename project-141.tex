...
\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  \large{Summary -- Part 6}
  \normalsize{}
  The goal was to design a controller such that it would cause the unstable system's output to decay to 0 
  in 0.05 seconds or less and without overshoot.
  In my solution, the linearized system from Part 5 was implemented in Simulink
  using a State-Space block with output $y = \delta v_{dc}$ and initial condition $\delta x(0) = \begin{bmatrix}
    8 & -8
  \end{bmatrix}^T$. A stiff solver(ode23s) was used. Since the open-loop transfer function $H(s) = \dfrac{10^8}{s^2-21.25s+10^8}$ is unstable, a proportional controller alone cannot stabilize the system because it does not change the sign of the s-term in the characteristic equation.
  Therefore, a PD controller was selected. With $K_p = 0, K_i = 0,$ and $K_d = 2.5*10^{-4}$, the closed loop characteristic polynomial becomes $s^2 + 24978.75s + 10^8$, which has two real negative poles. The corresponding response is stable, non-oscillatory, and settles to zero well within $0.05$s, satisfying the design requirements. 
\end{tcolorbox}
...
\textbf{Goal: }To implement the original nonlinear model in Simulink considering the safety requirements. Compare the linearized model with the nonlinearized model.\\ 

\textbf{Linearized Model: } 
\[\delta \dot{x} = A\delta x + B\delta u\] 
\[y = \delta x_2 = \delta v_{dc}\]
$\hookrightarrow$ Then, close the loop around a nonlinear model.

\

\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  Using the values restated in Part 6, the equilibrium state is 
  \[
  x_0 = \begin{bmatrix}i_{L0}\\ v_{dc0}\end{bmatrix} = \begin{bmatrix}0.33\\80\end{bmatrix}
  \]
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  Calculating for the initial condition:
  \[x(0) = x_0 + \delta x(0)\]
  \[\delta x(0) = \begin{bmatrix}
    8\\-8
  \end{bmatrix}
  \] 
  \[x(0) = \begin{bmatrix}
  ...
  \end{bmatrix}
  \]
  thus, the nonlinear system has an initial condition of:\\
  $\hookrightarrow i_L(0) = 8.33A$\\
  $\hookrightarrow v_{dc}(0) = 72V$
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  ...
  \begin{align*}
    ...
    v_s &= 80 + \delta v_s\\
    &\Downarrow\\
    \delta v_{dc} &= v_{dc} - 80
  \end{align*}
  Goal: drive $v_{dc} \rightarrow 80V$, driving $\delta v_{dc} \rightarrow 0$ (equilibrium).\\

  In the controller, \\
  $\hookrightarrow$ controller sees $v_{dc} - 80$\\
  $\hookrightarrow$ controller outputs $\delta v_s$\\
  $\hookrightarrow$ actual input to plant is $80 + \delta v_s$
\end{tcolorbox}

\

\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  \large{Nonlinear Dynamics}\normalsize{}\\
  Inductor current:
  \[\dot{i_L} = \dfrac{1}{L}(v_s-v_{dc})\]
  \[\dot{i_L} = 10000(v_s-v_{dc})\] 
  Capacitor voltage:
  \begin{align*}
    \dot{v_{dc}} &= \dfrac{1}{C}(i_L - \dfrac{v_{dc}}{R} - \dfrac{P}{v_{dc}})\\
    \dot{v_{dc}} &= 10000(i_L - \dfrac{v_{dc}}{1000} - \dfrac{20}{v_{dc}})\\
  \end{align*}
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  ...
  \end{minipage}\\
  \textbf{Figure 4: }The closed-loop Simulink model was built by placing the Part 6 Derivative-only controller around the nonlinear plant. Since the controller was designed using deviation variables, the feedback error for the nonlinear system was defined as $e(t) = 80 -v_{dc}(t)$, so that regulating the nonlinear output to $80V$ is equivalent to driving $\delta v_{dc} \rightarrow 0$. The controller output is $\delta v_s$, and the acutal plant input is formed using $v_s = 80 + \delta v_s$. A saturation block was added to enforce the source voltage safety bound as listed in the specification. Inside the plant subsystem, the nonlinear dynamics $\dot{i_L} = 10000(v_s-v_{dc})$ and $\dot{v_{dc}} = 10000(i_L -\dfrac{v_{dc}}{1000} - \dfrac{20}{v_{dc}})$ were implemented using gains, sums, divide, and integrator blocks. The state variables were also constrained to the required safety ranges $0 \leq i_L \leq 20 A$ and $0 \leq v_{dc} \leq 200V$. 
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  ...
  \end{minipage}\\
  \textbf{Figure 5: }Closed loop block diagram in Simulink of the nonlinear system with the Part 6 PD controller.  The capacitor voltage $v_dc$ is fed back and compared with the reference value of 80V to form the error $e(t) = 80 - v_{dc}(t)$. The controller generates the deviation input $\delta v_s$. A saturation block enforces the physical bounds $0 \leq v_s \leq 160V$. The nonlinear plant subsystem implements the original dynamics and outputs the inductor current and the capacitor voltage: $i_L$ and $v_{dc}$ respectively.
\end{tcolorbox}

\begin{tcolorbox}[enhanced, frame hidden, colback=white, borderline west={1pt}{0pt}{gray}]
  \large{Summary (Part 7)}\normalsize{}\\
  The original nonlinear system was implemented in Simulink using the plant equations for the inductor current and capacitor voltage, which include the constant power load term $P/v_{dc}$. The controller designed in Part 6 was originally formulated using deviation variables, so the nonlinear implementation required translating the controller back to the original variables. The feedback error was defined as $e(t) = 80-v_{dc}(t)$, ensuring that regulating the nonlinear output to 80V corresponds to driving the output $\delta v_{dc}$ to zero. The controller output produces $\delta v_s$, and the actual plant input is obtained using $v_s = 80 + \delta v_s$. \\

  I added saturation blocks to enforce the physical safety limits listed in the specifications in the project, ensuring that $0 \leq v_s \leq 160V$, $0 \leq i_L \leq 20A$, and $0 \leq v_{dc} \leq 200V$. Because the nonlinear model contains the term $\dfrac{P}{v_{dc}}$, a small lower bound was used in simulation to prevent division by zero. The system was simulated using the stiff solver \texttt{ode23s} to accurately capture the fast electrical dynamics. \\

  The nonlinear closed-loop system successfully regulates the capacitor voltage to the desired equilibrium value of 80V while keeping all signals within their required limits. The nonlinear response shows small differences in the transient behavior due to the true nonlinear nature of the constant power load. The controller I designed remains effective at stabilizing the system and restoring the system to its operating point, or to its equilibrium. 
\end{tcolorbox}

\newpage

\section{Project Summary}
In this project, I analyzed and controlled a DC power system supplying a constant power load (CPL) through an RLC network. The goal was to model the system, analyze its stability, and design a controller capable of stabilizing the system while meeting specified time domain performance requirements.\\

First, the system was modeled using nonlinear state-space equations with the inductor current and capacitor voltage: $i_L$ and $v_{dc}$ respectively selected as the system states. The constant power load introduces the nonlinear term $P/v_{dc}$, which makes the dynamics nonlinear and drastically affects system stability. \\

The system was then linearized around an equilibrium operating point using the Jacobian of the nonlinear state equations. This produced a linear state space representation of the form 
\[\delta \dot{x} = A\delta x + B\delta u\]
\\
Using the linearized model, the open-loop transfer function from the input voltage perturbation $\delta v_s$ to the output voltage perturbation $\delta v_{dc}$ was derived. Stability analysis showed that the constant power load behaves similarly to a negative resistance, meaning that if the power level is too large relative to the circuit parameters, the system becomes unstable.\\

The equilibrium operating point was then calculated using the given circuit parameters $R = 1000\Omega, P = 20W,$ and $v_{s0} = 80V$. The resulting equilibrium values were $v_{dc0} = 80V$ and $i_{L0} = 0.33A$. Using these values, the stability condition derived earlier was evaluated and it was shown that the linearized system is not BIBO stable.\\

To stabilize the system, a PD controller was designed using the linearized transfer function. A proportional controller alone could not stabilize the system because it cannot modify the sign of the damping term in the characteristic equation. By introducing derivative action, the controller changes the system dynamics so that all coefficients of the characteristic polynomial become positive. With controller parameters $K_p = 0$ and $K_d = 2.5*10^{-4}$, the closed-loop poles become real and negative, producing a stable over-damped response that satisfies the design requirements of no overshoot and a settling time below 0.05s. \\

The controller was then implemented in Simulink using the linearized state-space model. Simulation results showed that the controller successfully drives the output deviation $\delta v_{dc}$ back to zero within the required settling time while maintaining a smooth, non-oscillatory response.\\

Finally, the controller was applied to the original nonlinear system in Simulink. Because the controller was designed using deviation variables, the nonlinear implementation required translating the controller back to the original variables so that regulating $v_{dc}$ 80V corresponds to driving $\delta v_{dc}$ to zero. Saturation blocks were added to enforce the required physical limits on voltage and current. The nonlinear simulations confirmed that the controller stabilizes the system and regulates the capacitor voltage back to the equilibrium value while satisfying the safety constraints.\\

Overall, this project demonstrates how nonlinear power electronics systems with constant power loads can show destabilizing behavior, and how linearization combined with classical feedback control design can be used to stabilize and regulate such unstable systems. 

\section{Extra Figures}
\includegraphics[width = 0.7\textwidth]{extra1.png}\\
\includegraphics[width = 0.7\textwidth]{extra2.png}\\
